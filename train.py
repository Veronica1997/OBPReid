...
parser.add_argument('--data_dir', default='./dataset/data/train', type=str, help='training dir path')
...
parser.add_argument('--part_num', default=6, type=int, help='part number')
parser.add_argument('--mask_dir', default='./dataset/data/train/trainheatmap', type=str, help='attention heapmap dir path')
parser.add_argument('--hidden_dim', default=256, type=int, help='part number')
parser.add_argument('--result_dir', default='./res', type=str, help='path to save result')
...
def train_model(model, global_classifier, PCB_classifier, criterion,criterion_1, optimizer, scheduler, num_epochs=25):
    '''
    ...
    for epoch in range(num_epochs):
        ...
        for it, data in enumerate(dataloaders):
            ...
            if it % 390 == 0:
                ...
                # for i in range(part_size):
                print('Loss1_{}:{:.3f} ({:.3f})|Loss1_{}:{:.3f} ({:.3f})|Loss1_{}:{:.3f} ({:.3f})|Loss1_{}:{:.3f} ({:.3f}) |Loss1_{}:{:.3f} ({:.3f})|Loss1_{}:{:.3f} ({:.3f})|Loss2:{:.3f} ({:.3f}) \t'.format(1, running_loss[0].val, running_loss[0].avg, 2, running_loss[1].val, running_loss[1].avg, 3, running_loss[2].val, running_loss[2].avg, 4, running_loss[3].val, running_loss[3].avg, 5, running_loss[4].val, running_loss[4].avg, 6, running_loss[5].val, running_loss[5].avg,running_loss2.val, running_loss2.avg))
...
criterion = CrossEntropyLabelSmooth(class_num,reduce=False)
...
model = train_model(model, global_classifier, PCB_classifier, criterion,criterion_1, optimizer, exp_lr_scheduler,
                    num_epochs=120)
